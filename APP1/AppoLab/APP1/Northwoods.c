...
void seq_list_change(seq *s,char c1){
    ...
    if (index_c != -1 && index_c != s->longuer - 1) {
...
void decrypteSeq(seq *s1, char encrypted[], char decrypted[]) {
    ...
    for (int i = 0; i < len_enc; i++) {
        ...
        if (index_c == -1) {
            ...
            char next_char;
            if (index_c == s1->longuer - 1) {
                next_char = s1->tab[0];  
            } else {
                next_char = s1->tab[index_c + 1];  
            }

            decrypted[len_dec] = next_char;
            len_dec++;
            seq_list_change(s1, next_char);  
        }
    }
}
...
int main(){
    ...
    seq s4;
    s4.longuer = 0;
    char code[]="hasta la victoria siempre";
    ...
    envoyer("start");
    envoyer_recevoir(code,reponse);
    decrypteSeq(&s1,reponse,output);
    catch_password(output,password);
    envoyer_recevoir(password,reponse);
    
    decrypteSeq(&s2,reponse,output);
    crypteSeq(&s3,code2,output);
    envoyer_recevoir(output,reponse);
    crypteSeq(&s4,reponse,output);
